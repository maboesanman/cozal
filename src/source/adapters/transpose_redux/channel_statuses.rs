...
use crate::util::replace_mut::replace;
...
impl<'a, T: Transposer> RepeatStepFuture<'a, T> {
    pub fn poll(mut self, one_channel_waker: &Waker) -> Poll<CallerChannelStatus<'a, T>> {
        ...
        let next_status = match result {
            crate::transposer::step::StepPollResult::NeedsState => RepeatStepSourceState {
                caller_channel,
                step,
                block_reason,
            ...
            crate::transposer::step::StepPollResult::Pending => return Poll::Pending,
