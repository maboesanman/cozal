...
impl<T: Transposer> TransposeInner<T> {
    ...
    pub fn get_working_step(
        ...
        let i = self.steps.partition_point(|s| s.step.raw_time() <= time);
        ...
        loop {
            let step = steps.get_mut(i).map(|s| &mut s.step).unwrap();
            if !step.is_unsaturated() {
                break
            }
            i -= 1;
        }
        steps.get_mut(i).map(|s| &mut s.step).unwrap()
    }

    fn get_first_original_step_time(&self) -> Option<T::Time> {
        let step = &self.steps.back()?.step;
    ...
    pub fn handle_source_advance(&mut self, time: T::Time) {
        // throw away pending channels that were last polled before time
        todo!()
    }

    /// returns list of source channels to release
    pub fn handle_source_release_channel(&mut self, channel: usize) -> Vec<usize> {
        todo!()
    }
