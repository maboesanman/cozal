...
pub trait Source {
    ...
    /// The type of states emitted by the stream.
    type State: Sized;

    /// Attempt to retrieve the state of the stream at `time`, registering the current task for wakeup in certain situations.
    ///
    /// # Return value
    ///
    /// There are several possible return values, each indicating a distinct stream state for a time `t`:
    ///
    ...
    ///
    /// - [`Scheduled(state, t_s)`](EventStatePoll::Scheduled) indicates that All events at or before time `t` have been emitted, that the next event ready to be emitted is at time `t_s`, and that the state is ready for the requested time. Emitting `Scheduled` releases the stream of the responsibility of waking the current task for any new information which does not change any emitted states or create any events before time `t_s`. It is still responsible for waking the task on changes that affect events or states preceding `t_s`. The stream should be polled again at or after `t_s`, though what it means to be "at time t_s" is left to the consumer of the trait.
    ///
