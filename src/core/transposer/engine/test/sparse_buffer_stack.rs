...
fn basic_test() {
    ...
    for _ in 0..100 {
        ...
        assert!(stack_pin.as_mut().buffer(index, |s, b| *b = (s, b.1 + *s)).is_ok());
    }

    // buffer an item based on an old checkpoint
    assert!(stack_pin.as_mut().buffer(65, |s, b| *b = (s, b.1 + *s)).is_ok());

    // buffer an item which skips cloning
    assert!(stack_pin.as_mut().buffer(66, |s, b| *b = (s, b.1 + *s)).is_ok());

    // buffer an item which doesn't have the previous buffer available
    assert!(stack_pin.as_mut().buffer(69, |s, b| *b = (s, b.1 + *s)).is_err());

    // buffer a non existent item
    assert!(stack_pin.as_mut().buffer(1000, |s, b| *b = (s, b.1 + *s)).is_err());
