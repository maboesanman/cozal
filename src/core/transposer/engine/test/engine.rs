...
use crate::{core::{event_state_stream::{
            iter_event_state_stream::IterEventStateStream, EventStatePoll, EventStateStream,
            EventStateStreamExt,
        }, transposer::test::test_transposer::{HandleRecord, TestTransposer}}, test::test_waker::DummyWaker};
...
fn ordering_invariability() {
    ...
    engine_pin.as_mut().poll(35, &mut cx);
    engine_pin.as_mut().poll(35, &mut cx);
    engine_pin.as_mut().poll(35, &mut cx);
    engine_pin.as_mut().poll(35, &mut cx);
    engine_pin.as_mut().poll(35, &mut cx);
