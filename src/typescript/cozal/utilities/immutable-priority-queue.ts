...
type FilteredKeys<Base, Condition> = FilterFlags<Base, Condition>[keyof Base];
...
function heapExtract<T>(node: HeapNode<T>): {
  ...
  const removeDirection = (node.count).toString(2)[1] == "0"
    ? "Left"
    : "Right";
...
function heapGetBubbleDownDirection<T>(
  node: HeapNode<T>,
  compareKeys: number[] | undefined = undefined
): "Left" | "Right" | "Neither" {
  if (node === undefined) {
    return "Neither";
  }
  ...
  if (node.right !== undefined &&
    lessThan(node.right.keys, finalCompareKeys))
  {
...
function heapBubbleDown<T>(
  node: HeapNode<T>,
  replace: { keys: number[]; value: T; } | undefined = undefined
): HeapNode<T> {
  if (node === undefined) {
    return undefined;
  }
  ...
  switch (direction) {
    case "Left":
      return {
        ...
        left: heapBubbleDown(
          node.left,
          { keys: node.keys, value: node.value, ...replace }
        ),
        right: node.right
      };
    case "Right":
