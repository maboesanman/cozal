...
impl<T: Transposer> StepInputsEntry<T> {
    ...
    fn add_input<I: TransposerInput<Base=T>>(
        &mut self,
        time: T::Time,
        input: I::InputEvent,
    )
    where T: TransposerInputEventHandler<I>
    {
        ...
        let i = set.partition_point(|existing| {
            T::sort_input_events(time, &input, existing).is_lt()
        });
...
impl<T: Transposer> StepInputs<T> {
    pub async fn handle(
        &self,
        transposer: &mut T,
        cx: &mut dyn HandleInputContext<'_, T>,
    ) {
    ...
    pub fn add_event<I: TransposerInput<Base=T>>(
        &mut self,
        event: I::InputEvent
    )
    where T: TransposerInputEventHandler<I>
    {
        let step_inputs_entry = match self.inputs.entry(I::SORT) {
            std::collections::btree_map::Entry::Vacant(v) => v.insert(StepInputsEntry::new(self.time)),
